const faqPageData = [
    // 🟠 ALLMÄNT
    {
        category: "Allmänt",
        question: "Vad är Bachata Sensual?",
        answer: "Bachata Sensual är en modern stil av bachata med mjuka, flytande rörelser och starkt fokus på kontakt och musikalitet."
    },
    {
        category: "Allmänt",
        question: "Är kurserna öppna för alla åldrar?",
        answer: "Ja! Vi har elever från 18 till 60+. Alla är välkomna!"
    },

    // 🔵 PRAKTISKT
    {
        category: "Praktiskt",
        question: "Vad ska jag ha på mig?",
        answer: "Bekväma kläder du kan röra dig fritt i. Sneakers eller dansskor med glid fungerar bra – inget krav på speciella skor."
    },
    {
        category: "Praktiskt",
        question: "Behöver jag komma med partner?",
        answer: "Nej! Du kan komma själv. Vi roterar partners så att alla får dansa med alla."
    },

    // 🟢 KURSER OCH TIDER
    {
        category: "Kurser och tider",
        question: "Hur lång är varje kurs?",
        answer: "Kurserna är 8 veckor långa med 1 lektion i veckan på 1,5 timme."
    },
    {
        category: "Kurser och tider",
        question: "Har ni drop-in?",
        answer: "Nej, men första lektionen är alltid gratis så att du kan prova på innan du bestämmer dig."
    },

    // 🟣 PRISER OCH RABATTER
    {
        category: "Priser och rabatter",
        question: "Vad kostar kurserna?",
        answer: "1 kurs = 1600 kr, 2 kurser = 2600 kr, 3 kurser = 3600 kr, och så vidare. Se vår prislista för alla nivåer."
    },
    {
        category: "Priser och rabatter",
        question: "Jag är student – får jag rabatt?",
        answer: "Ja! Vi erbjuder 15% rabatt för studenter. Ta med giltigt student-ID."
    },
    {
        category: "Priser och rabatter",
        question: "Jag kan bara komma varannan vecka – får jag rabatt?",
        answer: "Ja, du får 35% rabatt om du bara kan delta varannan vecka."
        
    },
    {
        category: "Priser och rabatter",
        question: "Har ni parrabatt?",
        answer: "Ja, om ni anmäler er som par får ni 15% rabatt. Ni måste anmäla er tillsammans och till samma antal kurser, men det är inget krav att gå i samma kurs."
    },
    {
        category: "Priser och rabatter",
        question: "Kan man kombinera rabatter?",
        answer: "Nej, rabatter kan tyvärr inte kombineras. Du väljer den rabatt som passar dig bäst."
    },


    // 🔴 ANMÄLAN
    {
        category: "Anmälan",
        question: "Hur anmäler jag mig?",
        answer: "Via knappen på hemsidan – fyll i formuläret så får du en bekräftelse via mail inom 24 timmar."
    },
    {
        category: "Anmälan",
        question: "Kan jag anmäla mig till flera kurser?",
        answer: "Absolut! Du väljer enkelt flera kurser i anmälningsformuläret."
    }
];

export default faqPageData;
